...
The latest 2026-07-30 runtime acceptance passed all `91/91` Python gate smokes, `32/32` OpenClaw governance plugin tests, Python compileall, and the runtime-specific regression checks. Veyra now runs under the validated Conda interpreter `/opt/anaconda3/envs/veyra/bin/python` (`3.11.15`); local start and install scripts reject an implicit PATH interpreter, reject non-3.11 runtimes, and preflight pinned dependencies plus the macOS CA bundle before replacing the LaunchAgent. The regenerated LaunchAgent and its live process both use that interpreter, while OpenClaw kept PID `58174`. In the preceding exact-runtime acceptance run, Feishu established a current-run TLS connection, received two real websocket messages, processed them as `direct_answer / success` and `probe / verified_success`, and delivered both replies with `provider_sent` plus distinct external message ids; duplicate provider redelivery was suppressed without a second reply. The final transactional LaunchAgent process independently established a fresh TLS/WebSocket connection, delivered a provider-sent diagnostic message, then received and processed a real `direct_answer` message and returned a correlated `provider_sent` reply; the provider's duplicate redelivery was again suppressed. `/health` is consequently `degraded`, with only private Memory fallback, 5 historic reviews, and stale Beliefs remaining as operational notices rather than a Feishu TLS critical.
...
| P10 Runtime Stabilization and local release hardening | Implemented, live soak pending | Runtime trace recorder, telemetry summary APIs, ContextDriftDetector, ToolProxy guard smoke, first-stage route split, fail-closed Python 3.11 local install/start/status/reset scripts, atomic LaunchAgent replacement, truthful Feishu connection/readiness state, `.env.example`, GitHub smoke workflow, release-state boundary, first-stage Tauri desktop shell, and local setup API are implemented. Real Feishu/OpenClaw soak must validate stability under real user traffic. |
...
- Feishu live validation requires a real app ID/secret and either local WebSocket intake or a reachable callback URL. Full E2E additionally requires a current-run processed inbound message and a `provider_sent` reply with an external message id; `thread_alive` alone is not connection or delivery proof.
