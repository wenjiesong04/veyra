...
`ContextAnchorBinder` 在 canonical EventInbox admission 与 Understanding 完成后运行。普通聊天消息不要求、也不信任 caller 直接携带 `goal_id / case_id / task_id`；锚点由三段式链路补全：server 先从 exact-owner 可见的 active Goal、active/paused Commitment、Veyra 注册的同 session Task/Case 和未过期 Context Thread 构造最多 24 个 event-bound 候选；模型只看到 opaque token、kind、label、status 和 durable 标记并选择候选，不能提交真实 durable ID；Binder 最后重新校验 token、owner/session、kind、唯一 label、source quote 和候选快照。durable candidate 只有全部校验成立时才可绑定；没有可绑定对象时，只能从已验证 semantic act 建立 24 小时、same-user/same-session 的 `context_hypothesis`。该 hypothesis 不是 Entity、Goal、事实、因果或权限，不能跨 session 聚合；跨 session General Situation 仍必须有真实 active Goal 或 active/paused Commitment。

已存在 Context 的改写、翻译或指代续谈只允许一个更窄的复用路径：同一个 event-bound token 必须同时出现在 semantic target 的 `attributes.anchor_candidate_token` 与 turn-level `history_links`，且该候选仍是同 owner/session、未过期、非 durable 的 Context；缺任一字段都不能复用旧 Context。两个字段来自同一次模型响应，只是冗余结构一致性检查，不是两份独立证据，也不证明 referent 一定正确；复用结果仍是无事实、因果、路由、风险、执行或发送权的 hypothesis。
...
本 changeset 的确定性 fixture 另证明：第三个改写续谈事件可以通过上述双字段 token 复用旧 Context，使同一 General Situation 达到 3-child；只有 target token、缺少 `history_links` 的负例不会复用旧 Context。该 fixture 的 `3/3` 是契约回归，不替代上述真实 Kimi `3/6` 小样本，也不表示历史 40 个无 anchor Situation 已回填；旧记录仍保持原样。
...
真实对话还暴露并关闭两个泛化缺陷：Kimi 曾返回 `speaker=direct_user / authority=direct_user`，旧策略把结构化 actor 别名误判为第三方；现仅在 schema actor vocabulary 中接受 `direct_user`，reported/quoted/第三方 speaker 继续 fail closed。另一个旧路径把整个 target 控制对象串行化后做兼容匹配，`anchor_candidate_token` 字段名中的 `date` 曾错误选择 `time_probe`；现在 Probe 先按结构化 `evidence_need / target.type / operation` 判定，兼容层只读取 semantic values，控制字段名不再进入语义。进一步地，`external / fresh_external / fresh_runtime / fresh_local` 现在只描述证据缺口，不再自动授予公开搜索或通用 system probe；搜索必须来自明确的结构化 search operation、search target 或 search-specific evidence need，模型提供的 policy signal 也不能进入 server-owned `Decision.signals`。Veyra 自己确定性要求的具体只读 Probe 仍可保留。这关闭了盲目查公网和无关运行态探测，但属于自动化边界验证，不等于自然语言泛化已经完成。

同一轮重启验收又留下了一条先失败、后修复的真实语义证据。普通续谈“在不扩大执行权的前提下……这是架构讨论，请只分析”最初仍被投影为 `ASK_USER / R1`：第一层原因是编译器把任何未满足 `condition` 都当作待触发能力；修到 act-level 后，第二层原因又暴露为 effect classifier 会扫描自然语言 `goal/target`，于是主题中的“执行权”反而制造了 execution candidate。最终规则不再用描述性 goal/target 文本授予 effect：条件只阻断已经由结构化 act kind/operation/target shape 判定为 durable effect 或具体 Probe 的 act；read-only condition 只是无授权的回答约束。`condition / conditional / precondition` relation 的两端也纳入同一 fail-closed 判断，防止把前置条件移入关系图后绕过治理，而普通 `sequence` 不会误锁明确命令。修复后的新事件 `evt_cd846a958769` 为 `direct_answer / success / R1`，无 Probe、Agent、Tool 或 ExecutionTrace，Decision 与终态 `response_authority` 均为 `not_started / claim_scope=none`，不能声称查询、进展、完成或未来交付。该单条 Kimi live 最小对照只证明已知失败形状被结构化关闭，仍不等于条件句、否定或多轮指代已全面泛化。

同一 changeset 新增 terminal `response_authority.v1`：Controller 的每一个 return 都重建最终公开 Route、DecisionPlan、capability envelope 与 executor；effectful proposal 被治理层收窄为 Direct/Ask、Probe 或治理 Route 时，旧 Agent/Probe target、tool args、execution tier 和模型执行式 draft 不得残留。Awareness 最终只按 server-owned receipt 开放回答范围：无 receipt 为 `none`，持久 Agent dispatch 为 `dispatch_only`，终态 ExecutionTrace/只读观察最多为 `terminal_report`，只有与当前 server-owned 响应精确绑定的少量 `committed + applied` 内部状态变更才为 `verified_effect`。并行发生的 profile/memory write 不能证明 Agent/Probe 完成；没有 delivery receipt 永远不能承诺稍后自动交付。当前这条 effect 归并尚未通用接入 Tool Governance 的全部 `VerifiedToolEffect`，不能宣传为所有工具效果都已统一证明。

本 changeset 的最终自动化证据为 Python gate `127/127`、全部 9 Route 非弱化矩阵 `702/702`、OpenClaw governance plugin `32/32` 和完整 compileall。新增 terminal projection 专项覆盖 effectful candidate 的 Direct/Ask/Probe/governance 收窄、R3/R4/R5、ROLLBACK、无 receipt/dispatch/terminal report/exact state effect 四级回答权威、无关状态写入、未验证 Agent 完成摘要和禁止虚构未来交付；Context 专项覆盖 original Event immutability、A/B Situation 隔离、exact replay byte stability、dedupe phantom、disabled zero-write、跨 owner/候选歧义与最多 8 个 anchor；Semantic 专项覆盖 read-only constraint 与 conditional execution、act condition 与 relation-shaped precondition、conditional Probe 以及非条件 sequence 的最小对照；Cognitive 专项覆盖 55-owner continuity 与二次预算、source/state corruption、私有 status/trace 脱敏、config disable/ABA、stop/resume generation ABA、strict validation 后的最终持久化 fence 和 Active Loop 非阻塞。它们是 contract/fixture 证据，不替代真实自然语言 coverage、Kimi usefulness 或外部通知证据。
...
| 理解和决策 | `understanding_core.py`、`cognition_pipeline.py`、`semantic_policy.py`、`decision_core.py`、`final_route_projection.py`、`veyra_controller.py` | 模型优先理解、证据路由和多种执行路径已存在；描述性 goal/target 不能授予 effect，read-only constraint 与真正的条件执行/Probe 按 capability-sensitive act 和 condition relation 区分；最终 Route、DecisionPlan、capability envelope 与 `response_authority.v1` 由 server 统一投影，无 receipt 时不能声称查询、进展、完成或未来交付 | discussion-vs-command 与 terminal authority 只关闭已知边界；ExecutionTrace 仍不等于持久 effect，全部 Tool Governance effect 尚未统一归并；一次 Kimi live 和 fixture 通过不等于自然语言泛化完成，其他语言、否定、条件、引用和多轮指代仍需 held-out eval |
...
11. 最终公开 Route、Decision、DecisionPlan、capability envelope 与 response authority 必须一致，不能保留被拒绝候选的 executor、参数、执行 tier 或进度文案。
12. 没有 server-owned receipt 不得声称 dispatch、能力进度、完成或未来交付；Agent/模型成功文本和普通 terminal report 不是持久 effect 证明。
...
- `STRICT SEPARATION`：Extension coordinator 仍只能由 operator 使用私有控制面显式触发，不自动消费 Cognitive candidate、Context/General Situation、Attention proposal 或自然语言消息；`response_authority` 只约束前台结果可以说什么，不是 generation、validation、signature、canary 或 promotion receipt。完成 R0 pure-function 安全扩展链不等于完成主动认知产品链；
...
- `VALIDATED / AUTOMATED`：6.2d 当时的历史 gate 为 Python `95/95` 与 Route 矩阵 `270/270`；当前完整 changeset 的最终 gate 数字见 §1.14，本切片没有改变 generation/signing/canary/promotion authority；先前同一前端 changeset 的 Web Console build 与 macOS `.app` build 证据仍有效，本轮未改前端；
...
- 当前 Claim 已有 source、TTL、fresh/stale/conflict；完整 EvidenceGraph、实体关系和有效时间仍是目标，不能把嵌入式 evidence ref 写成已完成证据图；
- 只有成功解析 durable Goal/Commitment 或同 session Context anchor 时，当前关注才与长期情境发生结构化关联；否则保持 unknown，而不是猜测；
- 当前 Veyra 能在有界、结构化、信息型链中记录机会/问题 candidate，并在 `advise_only` 下投影到 exact-owner Console；跨领域主动调查、可靠互动时机、外部推送和持续用户可感知建议仍是 `TARGET`；
- Agent 提供开放认知和执行能力；
...
当前两条实际链必须诚实分开：`WorldState → private cognitive brief/candidate → 停止`，以及 `聊天事件 → Context sidecar → General Situation → record_only/Console → 停止`。要达到用户描述的长期动态智能，下一产品闭环仍需把受治理 ObservationRequest、世界变化验证、建议时机、exact-owner 触达、反馈和可撤销校准串起来；安全红线继续 deterministic，产品质量则用真实使用样本与人工反馈评估，不能靠放松 owner/evidence/authority 或重新堆关键词来制造“主动”。
