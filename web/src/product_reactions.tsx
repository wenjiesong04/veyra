...
export function ProductReactions({ reactions, scope, language = "zh", onChanged, compact = false }: Props) {
  ...
  const copy = en ? { eyebrow: "SUGGESTIONS", title: "Worth telling you now", empty: "Veyra is staying quiet for now.", happened: "What happened", relevant: "Why it matters", now: "Why now", recommendation: "Suggested next step", evidence: "Facts and inference stay separate in the Situation detail.", feedback: "Was this useful?", useful: "Useful", not_useful: "Not useful", ignore: "Ignore", resolved: "Resolved", too_early: "Too early", too_late: "Too late", too_frequent: "Too frequent", remind: "Remind before", remindButton: "Remind 1 day", days: "days", apply: "Apply", failed: "Feedback could not be saved." } : { eyebrow: "建议", title: "现在值得告诉你", empty: "Veyra 目前保持安静。", happened: "发生了什么", relevant: "为什么与你有关", now: "为什么是现在", recommendation: "我建议", evidence: "事实与推断会在 Situation 详情中分开显示。", feedback: "这条建议有帮助吗？", useful: "有帮助", not_useful: "没帮助", ignore: "不用管", resolved: "已解决", too_early: "太早了", too_late: "太晚了", too_frequent: "太频繁", remind: "提前提醒", remindButton: "提前 1 天", days: "天", apply: "应用", failed: "反馈暂时没有保存。" };
  const send = async (reaction: ProductReaction, label: ReactionFeedbackLabel, seconds?: number) => {
    ...
    if (!id || !Number.isFinite(revision) || revision < 1 || busy) return;
    setBusy(`${id}:${label}`); setError(null);
    try {
      await feedbackProductReaction(id, { user_id: scope.userId, session_id: scope.sessionId }, { label, situation_revision: revision, category: text(reaction.category, "general"), ...(seconds === undefined ? {} : { remind_before_seconds: Math.max(0, Math.min(30 * 86400, Math.round(seconds))) }) });
  ...
  return <section className={`productReactions ${compact ? "compact" : ""}`} aria-labelledby="product-reactions-title">
    ...
    {!visible.length ? <p className="productMuted">{copy.empty}</p> : <div className="productReactionList">{visible.slice(0, 8).map((reaction) => {
      ...
      return <article className="productReaction" key={id}>
        ...
        <div className="productFeedback"><span>{copy.feedback}</span><div className="productFeedbackButtons">{labels.map((label) => <button key={label} type="button" disabled={Boolean(busy)} className={label === "useful" ? "feedbackGood" : ""} onClick={() => void send(reaction, label)}>{label === "useful" ? <ThumbsUp size={13} /> : label === "not_useful" ? <ThumbsDown size={13} /> : null}{copy[label]}</button>)}<button type="button" disabled={Boolean(busy)} onClick={() => setReminderOpen(reminderOpen === id ? null : id)}><Clock3 size={13} />{copy.remindButton}</button></div></div>
