...
import { getProductStatus, getSetupStatus, type ProductContext, type ProductStatus } from "./api";
import { ProductSources } from "./product_sources";
import { statusLabel } from "./product_shared";
import type { OwnerScope } from "./shared";
import { asRecord, ErrorBlock, Freshness, isEnglish, LoadingBlock, Surface } from "./shared";

type Theme = "system" | "light" | "dark";
type Language = "system" | "zh" | "en";

export function Settings({ theme, language, scope, productContext, onTheme, onLanguage, onOpenSetup }: { theme: Theme; language: Language; scope: OwnerScope; productContext?: ProductContext | null; onTheme: (value: Theme) => void; onLanguage: (value: Language) => void; onOpenSetup: () => void }) {
  ...
  const copy = isEn ? { kicker: "Settings", title: "Make Veyra yours", intro: "Appearance, language, and local connections live here. Deep controls remain in Advanced / Labs.", appearance: "Appearance", appearanceHint: "Choose light, dark, or follow the system.", language: "Language", languageHint: "Switch between Chinese and English.", local: "Local setup", localHint: "Connect Agent, finish first-run setup, and Feishu intake.", permissions: "Local product boundary", permissionsHint: "This boundary shows what the local product may read and confirms that execution and external delivery remain off.", boundary: "This preview is local-only", boundaryHint: "Loopback and Tauri are supported. External delivery, execution, tool calls, and Agent dispatch remain off.", product: "Product", agent: "Agent", feishu: "Feishu", wizard: "Wizard", done: "Complete", pending: "Pending", openWizard: "Open setup wizard", advanced: "Advanced console", labs: "Labs", unknown } : { kicker: "设置", title: "让 Veyra 适合你", intro: "外观、语言和本机连接都在这里。深度能力仍保留在 Advanced / Labs。", appearance: "外观", appearanceHint: "选择浅色、深色或跟随系统。", language: "语言", languageHint: "中英文可切换，系统默认跟随浏览器。", local: "本机设置", localHint: "连接 Agent、完成首次启动和 Feishu 接入。", permissions: "本地产品边界", permissionsHint: "这里展示本机产品可以读取什么，并确认执行和外部交付仍保持关闭。", boundary: "本预览仅限本机", boundaryHint: "支持 loopback 与 Tauri；外部交付、执行、工具调用和 Agent 派发仍关闭。", product: "产品", agent: "Agent", feishu: "Feishu", wizard: "向导", done: "已完成", pending: "待完成", openWizard: "打开设置向导", advanced: "Advanced 旧控制台", labs: "Labs 实验能力", unknown };
  return <div className="sectionPage settingsPage"><div className="pageIntro"><div><span className="eyebrow">{copy.kicker}</span><h1>{copy.title}</h1><p>{copy.intro}</p></div></div><div className="settingsGrid"><Surface><div className="settingsHeading"><Palette size={18} /><div><h2>{copy.appearance}</h2><p>{copy.appearanceHint}</p></div></div><div className="choiceGroup">{(["system", "light", "dark"] as Theme[]).map((value) => <button key={value} className={theme === value ? "choice active" : "choice"} onClick={() => onTheme(value)}><span>{value === "system" ? <Settings2 size={16} /> : value === "light" ? <Sun size={16} /> : <Moon size={16} />}</span>{value === "system" ? (isEn ? "System" : "跟随系统") : value === "light" ? (isEn ? "Light" : "浅色") : (isEn ? "Dark" : "深色")}{theme === value ? <Check size={15} /> : null}</button>)}</div></Surface><Surface><div className="settingsHeading"><Languages size={18} /><div><h2>{copy.language}</h2><p>{copy.languageHint}</p></div></div><div className="choiceGroup">{(["system", "zh", "en"] as Language[]).map((value) => <button key={value} className={language === value ? "choice active" : "choice"} onClick={() => onLanguage(value)}>{value === "system" ? (isEn ? "System" : "跟随系统") : value === "zh" ? "中文" : "English"}{language === value ? <Check size={15} /> : null}</button>)}</div></Surface><Surface className="setupStatusCard"><div className="settingsHeading"><WandSparkles size={18} /><div><h2>{copy.local}</h2><p>{copy.localHint}</p></div><Freshness at={setupUpdatedAt} loading={setupLoading} error={setupError} language={language} /></div>{setupLoading ? <LoadingBlock /> : setupError ? <ErrorBlock message={setupError} onRetry={() => void load()} /> : <div className="setupFacts"><div><span>{copy.product}</span><strong>{String(desktop.product_name ?? "Veyra")}</strong></div><div><span>{copy.agent}</span><strong>{statusLabel(agent.status ?? "not_configured", isEn)}</strong></div><div><span>{copy.feishu}</span><strong>{statusLabel(feishu.readiness ?? "not_configured", isEn)}</strong></div><div><span>{copy.wizard}</span><strong>{wizard.completed === true ? copy.done : copy.pending}</strong></div></div>}<button className="primaryButton" onClick={onOpenSetup}><WandSparkles size={15} />{copy.openWizard}</button></Surface><Surface className="permissionsCard"><div className="settingsHeading"><LockKeyhole size={18} /><div><h2>{copy.permissions}</h2><p>{copy.permissionsHint}</p></div><Freshness at={statusUpdatedAt} loading={statusLoading} error={statusError} language={language} /></div>{statusError ? <ErrorBlock message={statusError} onRetry={() => void load()} /> : null}<div className="setupFacts"><div><span>{copy.product}</span><strong>{statusReadable ? statusLabel(productScope.mode ?? "local-first", isEn) : copy.unknown}</strong></div><div><span>{isEn ? "Sources" : "来源"}</span><strong>{statusReadable ? Object.keys(sources).length || "—" : copy.unknown}</strong></div><div><span>{isEn ? "External delivery" : "外部交付"}</span><strong>{statusLabel(externalDelivery, isEn)}</strong></div><div><span>{isEn ? "Execution" : "执行"}</span><strong>{statusLabel(execution, isEn)}</strong></div></div><p className="settingsBoundary"><strong>{copy.boundary}</strong><span>{copy.boundaryHint}</span></p></Surface><ProductSources scope={scope} productContext={productContext} language={language} /></div><div className="settingsLinks"><a href="#/advanced"><ExternalLink size={15} />{copy.advanced}</a><a href="#/advanced"><ExternalLink size={15} />{copy.labs}</a></div></div>;
}
