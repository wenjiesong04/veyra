...
import { getProductSources, type ProductContext, type ProductSources } from "./api";
...
export function ProductSources({ scope, productContext, language = "zh" }: { scope: OwnerScope; productContext?: ProductContext | null; language?: Language }) {
  ...
  const [pending, setPending] = useState<{ source: ReadableSourceId; mode: SourceConsentMode } | null>(null);
  ...
  const changeConsent = async (source: string, enabled: boolean) => {
    ...
    try {
      ...
      await applySourceConsent({ source, enabled, item, scope: currentContext.internalScope, en });
      await load();
      notifyProductConsentChanged();
      setPending(null);
    } catch (caught) {
