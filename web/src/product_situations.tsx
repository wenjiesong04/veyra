...
import { dateLabel, record, records, statusLabel, strings, text, uniqueRecords } from "./product_shared";
...
function epistemicLabel(value: unknown, en: boolean): { label: string; tone: string } | null {
  const normalized = text(value, "").toLowerCase();
  const labels: Record<string, [string, string]> = {
    reported: [en ? "Fact" : "事实", "fact"], observed: [en ? "Observed" : "已观察", "observed"],
    inferred: [en ? "Inference" : "推断", "inference"], verified: [en ? "Verified" : "已验证", "verified"],
  };
  const selected = labels[normalized];
  return selected ? { label: selected[0], tone: selected[1] } : null;
}
...
function evidenceProjection(value: unknown, en: boolean): EvidenceProjection {
  ...
  addField(en ? "status" : "状态", item.status);
  addField(en ? "freshness" : "新鲜度", item.freshness);
...
export function SituationsPage({ scope, language = "zh", onChanged }: ScopeProps) {
  ...
  const load = useCallback(async (signal?: AbortSignal) => { setLoading(true); setError(null); try { const result = await getProductSituations(scopeFor(scope), { signal }); const status = String(result.status ?? "unknown"); const freshness = record(result.freshness); setProjectionStatus(status); setFreshnessCategory(freshness.situations ?? "unknown"); setItems(uniqueRecords(result.items, { mode: "situation" }) as ProductSituation[]); setTerminal(uniqueRecords(result.recent_terminal, { mode: "situation" }) as ProductSituation[]); setUpdatedAt(new Date().toISOString()); if (status === "degraded") setError(en ? "Situation data is degraded; only verified rows are shown." : "Situation 数据部分不可用，只显示通过校验的内容。"); } catch (caught) { if (caught instanceof DOMException && caught.name === "AbortError") return; setProjectionStatus("unavailable"); setFreshnessCategory("unknown"); setError(caught instanceof Error ? caught.message : (en ? "Situations are unavailable" : "Situations 暂时不可用")); } finally { if (!signal?.aborted) setLoading(false); } }, [scope.userId, scope.sessionId, en]);
...
export function SituationDetailPage({ situationId, scope, language = "zh", onChanged }: ScopeProps & { situationId: string }) {
  const en = isEnglish(language);
  ...
  const load = useCallback(async (signal?: AbortSignal) => { setLoading(true); setError(null); try { const result = await getProductSituation(situationId, scopeFor(scope), { signal }); const status = String(result.status ?? "unknown"); const freshness = record(result.freshness); setProjectionStatus(status); setFreshnessCategory(freshness.situations ?? "unknown"); if (status === "degraded") { setDetail(null); setQuestions([]); setReactions([]); setError(en ? "This Situation is degraded; its state could not be verified." : "这条 Situation 部分不可用，状态未通过完整性检查。"); return; } if (!result.situation) throw new Error(en ? "Situation not found" : "Situation 不存在"); setDetail(result.situation); setQuestions(Array.isArray(result.questions) ? result.questions : []); setReactions(Array.isArray(result.reactions) ? result.reactions : []); } catch (caught) { if (caught instanceof DOMException && caught.name === "AbortError") return; setProjectionStatus("unavailable"); setFreshnessCategory("unknown"); setError(caught instanceof Error ? caught.message : (en ? "Situation is unavailable" : "Situation 暂时不可用")); } finally { if (!signal?.aborted) setLoading(false); } }, [situationId, scope.userId, scope.sessionId, en]);
  useEffect(() => { const controller = new AbortController(); void load(controller.signal); return () => controller.abort(); }, [load]);
