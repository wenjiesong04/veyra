...
import { BrainCircuit, ChevronLeft, ChevronRight, History, Settings, Sparkles } from "lucide-react";
...
type NavRoute = "home" | "matters" | "status" | "settings" | "advanced";
type Route = { kind: NavRoute } | { kind: "chat"; id: string };
const LegacyConsole = lazy(() => import("./LegacyConsole").then((module) => ({ default: module.LegacyConsole })));

function getRoute(): Route {
  const parts = window.location.hash.replace(/^#\/?/, "").split("/").filter(Boolean);
  const head = parts[0] ?? "";
  if (head === "chat" && parts[1]) return { kind: "chat", id: decodeURIComponent(parts[1]) };
  if (["matters", "status", "settings", "advanced"].includes(head)) return { kind: head as NavRoute };
  return { kind: "home" };
}
...
function AppShell() {
  ...
  useEffect(() => {
    ...
    const loadProductContext = async () => {
      ...
      try {
        ...
        setInputScope(external && typeof external.user_id === "string" && typeof external.session_id === "string" && typeof external.channel === "string" ? external : null);
      } catch {
        if (!cancelled) {
  ...
  const activeNav: NavRoute = route.kind === "chat" ? "home" : route.kind;
  const navItems: Array<{ id: NavRoute; label: string }> = [{ id: "home", label: "Veyra" }, { id: "matters", label: en ? "Matters" : "事项" }, { id: "status", label: en ? "Status" : "状态" }, { id: "settings", label: en ? "Settings" : "设置" }];
  const body = route.kind === "home" ? <HomePage scope={scope} productContext={productContext} inputScope={inputScope} language={language} onStartChat={startChat} onOpenHistory={() => setHistoryOpen(true)} />
    : route.kind === "chat" ? <ChatPage scope={scope} inputScope={inputScope} language={language} conversationId={route.id} historyEpoch={historyEpoch} pendingText={pendingChat?.id === route.id ? pendingChat.text : null} onPendingConsumed={() => setPendingChat(null)} onOpenHistory={() => setHistoryOpen(true)} onNewConversation={() => openRoute("home")} onHistoryChange={setHistory} />
    : route.kind === "matters" ? <Matters scope={scope} productContext={productContext} language={language} />
    : route.kind === "status" ? <Status scope={scope} language={language} />
    : route.kind === "settings" ? <SettingsPage theme={theme} language={language} onTheme={setTheme} onLanguage={setLanguage} onOpenSetup={() => setSetupOpen(true)} />
    : <div className="legacyPage"><div className="legacyHeader"><button className="textButton" onClick={() => openRoute("home")}><ChevronLeft size={15} />{en ? "Back to Today" : "返回 Today"}</button><span>Advanced · 旧控制台</span></div><Suspense fallback={<div className="loadingBlock">Loading Advanced…</div>}><LegacyConsole /></Suspense></div>;
  ...
  return <div className="veyraApp"><header className="appHeader"><a href="#/" className="brand" onClick={(event) => { event.preventDefault(); openRoute("home"); }}><span className="brandMark"><img src={iconPath} alt="" /></span><span>Veyra</span></a><nav className="desktopNav" aria-label={en ? "Main navigation" : "主导航"}>{navItems.map(({ id, label }) => <a key={id} className={activeNav === id ? "active" : ""} href={id === "home" ? "#/" : `#/${id}`} onClick={() => setRoute({ kind: id })}>{label}</a>)}</nav><div className="headerActions"><span className="scopeHint" title="请求会携带精确 owner/session">{en ? "Local" : "本机"}</span><button className="iconButton subtle historyButton" onClick={() => setHistoryOpen(true)} aria-label={en ? "Conversation history" : "会话历史"}><History size={17} /></button></div></header><main className="appMain">{body}</main><nav className="mobileNav" aria-label={en ? "Mobile navigation" : "移动端主导航"}>{navItems.map(({ id, label }) => <a key={id} className={activeNav === id ? "active" : ""} href={id === "home" ? "#/" : `#/${id}`} onClick={() => setRoute({ kind: id })}>{id === "home" ? <BrainCircuit size={18} /> : id === "matters" ? <Sparkles size={18} /> : id === "status" ? <ChevronRight size={18} /> : <Settings size={18} />}<span>{label}</span></a>)}</nav><HistoryDrawer open={historyOpen} onClose={() => setHistoryOpen(false)} items={history} language={language} onClear={clearCurrentHistory} onSelect={(item) => { setPendingChat(null); openChat(conversationIdFor(item)); }} /><SetupWizard open={setupOpen} onClose={() => setSetupOpen(false)} onComplete={async () => { setSetupOpen(false); setSetupStatus(await getSetupStatus()); }} setupStatus={setupStatus} coreModelStatus={coreModelStatus} agentStatus={agentStatus} /></div>;
}
